...
//import (
//	"github.com/linuxssm/frontendServer/authentication/routes"
//)
//
...
func Init() {
	...
	//r.HandleFunc("/wauth", handlePage).Methods("GET") // authorized page
...
func GetLogin(rw http.ResponseWriter, req *http.Request) {
	...
	fmt.Fprintf(rw, `
        ...
        <p><b>Messages: %v</b></p>
        <h3>Login</h3>
        <form action="/auth/loginPost" method="post" id="login">
...
var prefix string = ("/auth")
var redirectPath string = ("/auth/")

func PostLogin(rw http.ResponseWriter, req *http.Request) {
	username := req.PostFormValue("username")
	password := req.PostFormValue("password")
	if err := aaa.Login(rw, req, username, password, "/auth/main"); err == nil || (err != nil && strings.Contains(err.Error(), "already authenticated")) {
		http.Redirect(rw, req,redirectPath, http.StatusSeeOther)
	} else if err != nil {
		fmt.Println(err)
		http.Redirect(rw, req, prefix+"/login", http.StatusSeeOther)
	}
}
...
func PostAddUser(rw http.ResponseWriter, req *http.Request) {
	...
	http.Redirect(rw, req, prefix+"/admin", http.StatusSeeOther)
}
		...
		t, err := template.New("page").Parse(`
            ...
            <body>
                ...
                {{ with .User }}
                    ...
                    <p>{{ if .Role | eq "admin" }}<a href="/auth/admin">Admin page</a> {{ end }}<a href="/auth/logout">Logout</a></p>
                {{ end }}
                <form action="/auth/change" method="post" id="change">
                    <h3>Change email</h3>
	...
	if user, err := aaa.CurrentUser(rw, req); err == nil {
		...
		t, err := template.New("admin").Parse(`
            ...
            <body>
                ...
                <form action="/auth/add_user" method="post" id="add_user">
                    <h3>Add user</h3>
